...
export default {
  ...
  effectDescription: {
    lightAndShadow: '根据一天中的时间和地理位置模拟逼真的阳光照射和阴影投射。可调节阴影强度、阳光和环境光颜色。',
    ink: '应用水墨艺术风格，使边缘变暗并创建手绘外观。调整强度以控制效果的程度。',
    brightnessContrast: '调整地图的整体亮度和对比度。使用正值来增加亮度或对比度，负值来变暗或平坦化图像。',
    hueSaturation: '改变色调并调整整个地图的饱和度。适用于创建颜色主题或降低视图饱和度。',
    vibrance: '选择性地增强暗淡颜色的强度，而不会使已经鲜艳的颜色过度饱和。比饱和度产生更自然的颜色增强效果。',
    sepia: '应用一种温暖的棕色调，让人联想到老照片。控制混合量以在原始颜色和怀旧色调之间切换。',
    dotScreen: '将图像转换为单色点图案，类似于报纸半色调印刷。调整角度、点大小和中心位置。',
    colorHalftone: '模拟CMYK彩色半色调印刷，每个颜色通道使用单独的点图案。控制角度、点大小和中心位置。',
    ...
    tiltShift: '模拟移轴镜头效果，模糊焦点带之外的区域，创造微缩模型的外观。通过起始/结束位置设置焦点带。',
    ...
    hexagonalPixelate: '将图像替换为六边形瓦片网格，每个瓦片填充其覆盖区域的平均颜色。调整瓦片缩放比例。',
    distanceFog: '根据物体到相机的深度将远处物体淡入雾色，增强纵深感。控制密度、起始距离、范围和雾色。',
  ...
  modal: {
    ...
    exportMap: {
      ...
      html: {
        ...
        tokenSecurityWarning: '* 警告：您的Mapbox令牌将嵌入导出的HTML文件中。任何有权访问此文件的人都可以看到并使用您的令牌。请尽可能使用带有URL限制的范围令牌。',
